...
6. Elegir de [`@marraqueta/ui-modules`](https://github.com/marraqueta-lab/ui-modules)
...
7. Correr el `schema.sql` de cada módulo en Supabase, **en orden de
   dependencia** (primero los que no requieren nada — ver la tabla en el
   README de `ui-modules`).

8. Montar cada panel en su ruta. El README de cada módulo trae el snippet
   exacto. Antes de construir cualquier componente de negocio desde cero,
   revisar si ya existe en la biblioteca.

### 4. Diseño y contexto

9. Completar `CLAUDE.md` con el contexto real del proyecto.
10. Completar `DESIGN.md` (etapa 0.5) antes de escribir el primer componente.

### 5. Deploy y CI

11. Conectar un **proyecto de Vercel propio** para este cliente, con su
    dominio. Cargar ahí las variables de entorno.
12. Activar branch protection en `main` con el check `build-and-test`.
13. Opcional: definir los secretos `NEXT_PUBLIC_SUPABASE_URL` y
